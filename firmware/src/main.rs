...
fn main() -> anyhow::Result<()> {
    ...
    let month_last_day = get_last_day_of_month(today.year(), today.month())
        ...
        .unwrap()
        .with_timezone(&Utc);
    println!("Last day of month: {:#?}", month_last_day);
    ...
    let activities = {
        ...
        for url in env!("ICS_URL").split(";") {
            ...
            events.extend(
                ics_downloader
                    .download_and_parse_ics()
                    .map_err(|e| anyhow::anyhow!("Failed to download ans parse ICS: {:?}", e))?,
            );
        }
        events.sort();
...

    {
        ...
        // Wait 1 second for the display to power on
